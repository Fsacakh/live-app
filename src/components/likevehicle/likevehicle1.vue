...
<template>
    <div class="hello">
        ...
        <div class="carstyle-header">
            <ul class="clearfix">
                <li v-for="item in crumb" @click="bread(item)"><i class="carname active">{{item.name}}</i><span class="iconfont icon-icon-tiaozhuanjiantou"></span></li>
                <li><i class="carname ">{{crumblastname}}</i></li>
            </ul>
        </div>
        <div class="carstyle-content">
            <ul>
                <li v-for="item in factorylist " :class="{'active':item.active}" @click="selectfactory(item)"><i>{{item.factoryName}}</i><i class="num" v-if="GroupcustomizationValue != '000000'">{{item.nums || 0}} 台</i></li>
                <li v-for="item in carbrandlist " :class="{'active':item.active}" @click="selectbrand(item)"><i>{{item.brandName}}</i><i class="num" v-if="GroupcustomizationValue != '000000'">{{item.nums || 0}} 台</i></li>
                <li v-for="item in cartrainlist " :class="{'active':item.active}" @click="selecttrain(item)"><i>{{item.seriesName}}</i><i class="num" v-if="GroupcustomizationValue != '000000'">{{item.nums || 0}} 台</i></li>
                <li v-for="item in cartypelist " :class="{'active':item.active}" @click="selecttype(item)"><i>{{item.modelName}}</i><i class="num" v-if="GroupcustomizationValue != '000000'">{{item.nums || 0}} 台</i></li>
                <li v-for="item in carstylelist " :class="{'active':item.active}" @click="selectstyle(item)"><i>{{item.displayName}}</i><i class="num" v-if="GroupcustomizationValue != '000000'">{{item.nums || 0}} 台</i></li>
            </ul>
        </div>
