...
<div>
    <transition name="fade">
        <div class="select-box" v-if="show" ref="select">
            <div class="m-head" v-if="hasHead">
                <div class="ns_left" @click="close">{{cancelText}}</div>
                <div class="ns_right" @click="confirm">{{confirmText}}</div>                
            </div>
            <div class="item-box">
                <!-- <div class="item" v-for="(item, index) in items" @click="itemClick(item)" :key="index">
                    {{item.text}} 
                </div> -->
    ...
    export default {
        ...
        methods: {
            ...
            valuesChange(picker, val) { 
                ...
                // console.log(this.slots[0].defaultIndex, this.slots[0].value)

                // this.slots[0].values.forEach(item => {
